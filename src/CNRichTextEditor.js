...
class CNRichTextEditor extends Component {
  ...
  removeImage = (index) => {
    ...
    if (content.component === 'image') {
      ...
      if (
        ...
        if (this.textInputs[index + 1].textLength > 0 && nextContent.content.length > 0) {
  ...
  handleMeasureContentChanged = (content) => {
    this.setState({ measureContent: content });
  };
  ...
  onRootLayout = (event) => {
    ...
    const paddingTop = (style && style.padding)
      ? style.padding
      : (style && style.paddingTop)
        ? style.paddingTop
        : 10;
    const paddingBottom = (style && style.padding)
      ? style.padding
  ...
  renderInput = (input, index, isLast, measureScroll = true) => {
    ...
    return (
      <View
        ...
        style={{ flexGrow: isLast === true ? 1 : 0 }}
      >
        <CNTextInput
              ...
              IS_IOS && measureInputScroll && (
                ...
                >
                  <CNTextInput
                    ...
                    styleList={styleList}
                    style={{ width: this.state.layoutWidth }}
                    crossPlatformFonts={this.props.crossPlatformFonts}
                  />
                </View>
              )
